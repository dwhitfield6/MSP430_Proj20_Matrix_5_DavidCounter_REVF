...
int main(void)
{
    ...
    TACCR0 = 32767;               // f_xtal / 2 - 1 gives 2 Hz
  ...
  while(1)
  {
	  ...
	  if(dayssecond !=48)
	  {
		  timeString[i++] = dayssecond;
			  timeString[i++] = daysfirst;
	  }
	  else
	  {
		  timeString[i++] = daysfirst;
	  ...
	  if(hourssecond !=48)
	  {
		  timeString[i++] = hourssecond;
			  timeString[i++] = hoursfirst;
	  }
	  else
	  {
		  timeString[i++] = hoursfirst;
	  ...
	  if(minutessecond !=48)
	  {
		  timeString[i++] = minutessecond;
			  timeString[i++] = minutessfirst;
	  }
	  else
	  {
		  timeString[i++] = minutessfirst;
  ...
  //  setText("DAVID    "); // abcdefghijklmnopqrstuvwxyz 0123456789 (37) heart  !	  smilly	  boner1 	  boner2	  eyes right	  eyes center	  eyes left
	  setText(timeString);
...
for(j=0; j < digits;j++)
{
    ...
    {
      jjjjjj =  textBuffer[j-4];
    }

    printmatrix(jjjjjj,jjjjj,jjjj,jjj,jj);
    _delay_cycles(200000);
  }
setTime();
  setText(" (  "); // abcdefghijklmnopqrstuvwxyz 0123456789 (37) heart  !	  smilly	  boner1 	  boner2	  eyes right	  eyes center	  eyes left
  printmatrix(0,0,44,0,0);
  _delay_cycles(400000);
  printmatrix(0,0,42,0,0);
  _delay_cycles(400000);
  printmatrix(0,0,44,0,0);
  _delay_cycles(400000);
  printmatrix(0,0,42,0,0);
